...
## 1. 개요
이 문서는 `PROJECT_PLAN.md` 기획서를 기준으로 현재 코드베이스의 구현 상태를 점검하고, 코드 품질, 구조, 성능 관점에서 개선이 필요한 부분을 분석하여 리팩터링을 제안합니다.
...
### 2.1. 잘 구현된 기능
- **회원 관리 및 인증**: `Spring Security`를 사용하여 세션 기반 로그인, 회원가입, 프로필 관리 기능이 기획대로 안정적으로 구현되었습니다.
- **일반 공구/게시글 CRUD**: 공동구매와 커뮤니티 게시글의 기본적인 생성, 조회, 수정, 삭제 기능이 `URL_DESIGN.md`에 맞게 잘 구현되어 있습니다.
- **통합 댓글 시스템**: `CommentController`에서 `@RequestParam`으로 `targetType`과 `targetId`를 받아 공구와 게시글의 댓글을 단일 엔드포인트(`/comments`)로 처리하는 확장성 있는 구조가 구현되었습니다.
- **후기 및 매너온도**: `ReviewService`에서 후기 작성 시 주최자의 매너온도가 변동되는 로직이 기획대로 구현되었습니다.
- **알림 및 뱃지/포인트**: 각 도메인 서비스의 비즈니스 로직(공구 생성, 참여, 후기 작성 등)이 실행될 때 `NotificationService`, `BadgeService`, `PointService`를 호출하여 관련 기능이 잘 연동되어 있습니다.

### 2.2. 구현이 부족하거나 기획과 다른 부분
- **레시피 데이터 캐싱**: `BACKLOG.md`의 **최우선 과제**임에도 불구하고, `RecipeService`는 외부 API(`TheMealDB`, `식품안전나라`)를 매번 실시간으로 호출하고 있습니다. 이는 API 호출 제한(Rate Limit), 응답 속도 저하, 외부 API 장애 시 서비스 중단으로 이어질 수 있는 심각한 문제입니다.
...
- **조회수 중복 증가**: `Post` 엔티티에 `viewCount` 필드는 있으나, `BACKLOG.md`에 명시된 대로 세션 기반의 중복 카운트 방지 로직이 구현되지 않아 새로고침 시마다 조회수가 계속 증가합니다.
...
- **제안**:
    1. **엔티티에 비즈니스 로직 위임**: `GroupBuyService`의 공구 생성, 수정, 참여, 취소 관련 핵심 로직을 `GroupBuy` 엔티티 내부의 메서드로 옮깁니다. 서비스는 트랜잭션을 관리하고 엔티티의 메서드를 호출하는 역할에 집중해야 합니다.
        ```java
        // GroupBuy.java
        public void participate(User user, Participation newParticipation) {
            if (this.isHost(user)) {
...
- **제안**:
    1. **도메인 이벤트 발행**: `Spring ApplicationEventPublisher`를 사용하여 도메인 이벤트를 발행하는 구조로 리팩터링합니다.
        - `GroupBuyService`는 공구를 생성한 후 `GroupBuyCreatedEvent`를 발행합니다.
        - `BadgeService`, `PointService`, `NotificationService`는 각각 이 이벤트를 구독(`@EventListener`)하여 자신의 책임에 맞는 로직(뱃지 수여, 포인트 적립 등)을 비동기적 또는 동기적으로 처리합니다.
        - 이를 통해 각 서비스 간의 **결합도(Coupling)를 크게 낮출 수 있습니다.**
    2. **유틸리티 및 헬퍼 분리**: `ImageUploadUtil`처럼, 도메인과 직접 관련 없는 부가 기능은 별도의 유틸리티 클래스로 분리하는 것이 좋습니다.
...
- **제안**:
    ...
    2. **DTO 직접 조회 (QueryDSL)**: QueryDSL을 사용하여 처음부터 필요한 데이터만 DTO로 직접 조회하면, 엔티티 매핑 과정 없이 성능을 최적화할 수 있으므로 복잡한 조회 화면에 가장 적합한 방식입니다.
...
- **문제점 1: 게시글 수정 시 카테고리 미반영 (`BACKLOG.md`)**
    ...
    - **제안**: `editPage` 메서드에서 `UpdatePostRequest`를 채울 때, `post.getCategory()` 값을 함께 설정해주어야 합니다.
        ```java
...
2.  **`GroupBuy` 도메인 리팩터링**: 서비스 계층의 비즈니스 로직을 `GroupBuy` 엔티티로 옮겨 풍부한 도메인 모델을 구축하고, 이벤트 기반 아키텍처를 도입하여 서비스 간 결합도를 낮추는 작업을 진행합니다.
